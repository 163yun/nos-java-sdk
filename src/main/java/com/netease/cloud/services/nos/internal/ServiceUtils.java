...
public class ServiceUtils {
	...
	public static void downloadObjectToFile(NOSObject Object, File destinationFile) {
		...
		// if
		// (ServiceUtils.isMultipartUploadETag(Object.getObjectMetadata().getETag())
		// == false) {
		// byte[] clientSideHash = ServiceUtils.computeMD5Hash(new
		// FileInputStream(destinationFile));
		// byte[] serverSideHash =
		// BinaryUtils.fromHex(Object.getObjectMetadata().getETag());
		//
		// if (!Arrays.equals(clientSideHash, serverSideHash)) {
		// throw new
		// ClientException("Unable to verify integrity of data download.  " +
